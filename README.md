...
### `pi-games.typ` — Normal form games

Draws strategic (normal) form payoff matrices using [CeTZ](https://github.com/cetz-package/cetz). Supports 2-player and 3-player games.
...
Draws extensive form (sequential) game trees inside a `cetz.canvas` environment. Inspired by the `xgames` LaTeX package.
...
**Global style variables** (override after import): `game-pal`, `game-nature-color`, `game-fg`, `game-highlight-color`, `game-infoset-color`, `game-subgame-stroke/fill/label`, and geometry constants `game-R`, `game-gap`, `game-act`, `game-apos`, `game-tick`, stroke widths `game-sw-*`, font sizes `game-fs*`.

---

### `pi-game-palette.typ` — Shared colour palette

Single source of truth for all colours used by both libraries. Imported automatically by `pi-games.typ` and `pi-game-trees.typ`. Can be imported standalone to access colours in document text.

Defines: `pi-player-colors` (5-entry array), `pi-nature-color`, `pi-fg`, `pi-nash-color`, `pi-highlight-color`, `pi-infoset-color`, `pi-subgame-stroke`, `pi-subgame-fill`, `pi-subgame-label`.
...
This file is independent of `pi-games.typ` and `pi-game-trees.typ` and is only needed when using `fletcher` for game trees.
...
| File | Description |
|---|---|
| `pi-games-example.typ` | Usage examples for `pi-games.typ`: Prisoner's Dilemma, Battle of the Sexes, three-player games, best responses, and Nash equilibrium highlights |
| `pi-games-example.pdf` | Compiled output of the above |
| `pi-games-manual.typ` | Auto-generated API reference for `pi-games.typ` (built with [tidy](https://github.com/Mc-Zen/tidy)) |
| `pi-games-manual.pdf` | Compiled API reference for `pi-games.typ` |
| `pi-game-trees-example.typ` | Usage examples for `pi-game-trees.typ`: perfect-information trees, information sets, Nature nodes, equilibrium highlights, subgame markers |
| `pi-game-trees-example.pdf` | Compiled output of the above |
| `pi-game-trees-manual.typ` | Auto-generated API reference for `pi-game-trees.typ` (built with tidy) |
| `pi-game-trees-manual.pdf` | Compiled API reference for `pi-game-trees.typ` |
...
|---|---|---|
| `@preview/cetz` | 0.5.2 | `pi-games.typ`, `pi-game-trees.typ` |
